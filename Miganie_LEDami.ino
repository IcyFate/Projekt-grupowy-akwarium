...
const int resolution = 255;   // Zmienna przechowująca maksymalną wartość dla sygnału PWM
...
bool sensorOnOff = false;   // Zmienna określająca czy czujnik jest włączony czy wyłączony

void setup() {
  Serial.begin(115200);   // Szybkość komunikacji między ESP a portem szeregowym
...
void loop() {
  ...
  if (Serial.available() > 0) {   // Jeśli port szeregowy jest dostępny
    ...
    sensorOnOff = Serial.parseInt();    // Zapisanie wartości 0 lub 1 określające włączenie lub wyłączenie czujnika
  }

  if(sensorOnOff == true){      // Wykonuje się gdy czujnik jest włączony
    ...
    analogWrite(bluePin, blue);
     ...
     analogWrite(redPin, redS);   // Nadanie odpowiedniej wartości poprzez sygnał PWM do odpowiednich wyprowadzeń R, G i B paska LED
     analogWrite(greenPin, greenS);
     analogWrite(bluePin, blueS);
  }
}
